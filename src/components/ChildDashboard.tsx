...
const ChildDashboard: React.FC<ChildDashboardProps> = ({ 
  ...
  const progressPercentage = totalAvailable > 0 ? (completedToday.length / totalAvailable) * 100 : 0;
  ...
  return (
    <div className="animate-fade-in">
      <Header
        title={`Salut ${child.name}! ${child.avatar}`}
      ...
      <div className="bg-white rounded-2xl p-6 shadow-lg mb-8">
        ...
        <div className="mb-4">
          <div className="flex justify-between text-sm font-medium text-gray-600 mb-2">
            ...
            <span>{completedToday.length}/{totalAvailable}</span>
          </div>
        ...
        {completedToday.length > 0 && (
          <div className="mt-4">
            <h4 className="font-bold text-gray-700 mb-2">Activități completate:</h4>
            <div className="space-y-2">
              {completedToday.map(ca => (
                <div key={ca.id} className="flex items-center justify-between bg-green-50 p-3 rounded-lg">
                  <div className="flex items-center gap-2">
                    <span className="text-lg">{ca.activity?.emoji}</span>
                    <span className="text-gray-700">{ca.activity?.name}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="text-green-600 font-bold">+{ca.points_earned} puncte</span>
      ...
      {selectedDate === new Date().toISOString().split('T')[0] && (
        <div>
          <h3 className="text-2xl font-bold text-gray-800 mb-6">Activități disponibile astăzi:</h3>
